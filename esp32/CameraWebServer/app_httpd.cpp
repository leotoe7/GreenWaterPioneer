...
static esp_err_t gwp_handler(httpd_req_t *req)
{
    //static char html_response[1024];

    //sensor_t *s = esp_camera_sensor_get();
    //char *p = html_response;
    //p += sprintf(p, "<!doctype html><html lang=\"zh\"><head><meta charSet=\"utf-8\" /><title data-rh=\"true\">\u8fd9\u662f\u4e00\u4e2a\u6d4b\u8bd5html\u7f51\u9875</title></head><body><h1>\u8fd9\u662f\u4e00\u4e2a\u6d4b\u8bd5html\u7f51\u9875</h1></body></html>");
    //p += sprintf(p, "%s", uart_data);
    //*p++ = 0;
...
void startCameraServer()
{
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        httpd_register_uri_handler(camera_httpd, &gwp_uri);
